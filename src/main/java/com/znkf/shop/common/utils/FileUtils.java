...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	/**
	 * @author 武海升
	 * @desc 生成图片存储路径以及数据库保存路径
	 * @param multipartFile 文件
	 * @param businessNamePath 业务类型
	 */
	public static JSONObject setFilenamePath(MultipartFile multipartFile, String businessNamePath){
		String filenamePath;
		String filename = multipartFile.getOriginalFilename();
		String extension = FilenameUtils.getExtension(filename);
		String basename = UUID.randomUUID().toString();
		Calendar now = Calendar.getInstance();
		filename = now.get(Calendar.YEAR)+"_"+(now.get(Calendar.MONTH) + 1)+ "_"+ basename + "." + extension;
		String path = ConfigConstants.UPLOAD_PICTURE_FILE_PATH+businessNamePath;
		System.out.println(path);
		filenamePath = path + "/" + filename;
		String returnFilenamePath = businessNamePath+"/" + filename;
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("filenamePath",filenamePath);
		jsonObject.put("returnFilenamePath",returnFilenamePath);
		return jsonObject;
